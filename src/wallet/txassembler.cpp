    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional XCL outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes XCL and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::XCL_TO_XCL) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a XCL-to-XCL transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::XCL_ONLY;
        ...
        // If XCL-to-XCL fails, create a peg-out transaction
