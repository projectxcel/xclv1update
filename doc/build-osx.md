...
1. Clone the ProjectExcelsior Core source code:
    ```shell
...
2.  Build ProjectExcelsior Core:
    ...
    Configure and build the headless ProjectExcelsior Core binaries as well as the GUI (if Qt is found).
...
## Disable-wallet mode
When the intention is to run only a P2P node without a wallet, ProjectExcelsior Core may be
...
```shell
mkdir -p "/Users/${USER}/Library/Application Support/ProjectExcelsior"

touch "/Users/${USER}/Library/Application Support/ProjectExcelsior/projectexcelsior.conf"

chmod 600 "/Users/${USER}/Library/Application Support/ProjectExcelsior/projectexcelsior.conf"
```

The first time you run projectexcelsiord, it will start downloading the blockchain. This process could
