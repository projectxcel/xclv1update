...
- [Development guidelines](#development-guidelines)
    - [General ProjectExcelsior Core](#general-projectexcelsior-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
ProjectExcelsior Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
ProjectExcelsior Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/projectexcelsior-project/projectexcelsior/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_projectexcelsior
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_projectexcelsior --log_level=test_suite
$ valgrind -v --leak-check=full src/projectexcelsiord -printtoconsole
...
# A coverage report will now be accessible at `./test_projectexcelsior.coverage/index.html`.
```
...
To profile a running projectexcelsiord process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep projectexcelsiord` -- sleep 60
```
...
ProjectExcelsior Core can be compiled with various "sanitizers" enabled, which add
...
file descriptors, it will cause problems with ProjectExcelsior's `select()` loop, because
...
```bash
$ lsof -p $(pidof projectexcelsiord) |\
...
compatibility between nodes. This happened in ProjectExcelsior 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to ProjectExcelsior's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of ProjectExcelsior Core, the wallet was always
